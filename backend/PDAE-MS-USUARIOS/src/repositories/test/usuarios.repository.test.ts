...
describe('UsuariosRepository', () => {
  ...
  it('ejecutarCrearUsuario llama a sp_crear_usuario con type', async () => {
    ...
    expect(espia).toHaveBeenCalledWith('sp_crear_usuario', ['CUSTOMER'], undefined);
  });
  ...
  it('ejecutarListarUsuarios llama a sp_listar_usuarios con orden de parámetros', async () => {
    ...
    expect(espia).toHaveBeenCalledWith('sp_listar_usuarios', ['5', 'ADM', 20, 2], undefined);
  });
  ...
  it('ejecutarListarUsuarios pasa null en filtros omitidos', async () => {
    ...
    expect(espia).toHaveBeenCalledWith('sp_listar_usuarios', [null, null, 10, 1], undefined);
  });
  ...
  it('ejecutarActualizarUsuario llama a sp_actualizar_usuario', async () => {
    ...
    expect(espia).toHaveBeenCalledWith('sp_actualizar_usuario', ['3', 'ADMIN'], undefined);
  });
});
