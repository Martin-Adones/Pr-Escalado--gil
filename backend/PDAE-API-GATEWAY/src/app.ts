...
const MS_USUARIOS_URL = microservices.find((s) => s.prefix === "usuarios")?.url;
...
export const createServer = async (): Promise<FastifyInstance> => {
    ...
    try {
      ...
      spec.paths["/api/health"] = {
        get: {
          ...
          description:
            "Chequeo de salud consolidado del Gateway y sus microservicios asociados.",
          tags: ["Sistema"],
          responses: {
            200: {
            ...
            503: {
              description:
                "Algún microservicio o dependencia se encuentra caído (DEGRADED)",
            },
          },
        },
      };
  ...
  app.addHook("preHandler", async (request, reply) => {
    ...
    try {
      ...
      const role = clientRoles.includes("p10-admin")
        ? "admin"
        : clientRoles.includes("p10-client")
          ? "client"
          : null;
  ...
  for (const service of microservices) {
    app.log.info(
      `[Proxy] Configurando redirección /api/${service.prefix}/* -> ${service.url}/api/${service.prefix}/*`,
    );
