CREATE OR REPLACE FUNCTION sp_listar_logs_auditoria(
  ...
  p_assignet_to TEXT DEFAULT NULL,
...
BEGIN
  ...
  RETURN QUERY
  WITH filtrados AS (
    SELECT a.id_audit_logs, a.id_contracts, a.action, a.assignet_to, a.created_at
    FROM audit_logs a
    WHERE (p_id_audit_logs IS NULL OR a.id_audit_logs = p_id_audit_logs::BIGINT)
      ...
      AND (p_assignet_to IS NULL OR a.assignet_to ILIKE '%' || p_assignet_to || '%')
